...
function trendWidget(
  ...
  return {
    id: defaultWidgetId('trend.chart'),
    type: 'trend.chart',
    x,
    y,
    w,
    h,
    config: { dataset, metric, days },
  };
}
...
export function buildDefaultDashboardLayout(roles: string[] | undefined): DashboardLayout {
  ...
  if (fulfiller) {
    ...
    y += 2;
    widgets.push(trendWidget('incidents', 'opened', 30, 0, y, 6, 4));
    widgets.push(trendWidget('incidents', 'open_backlog', 30, 6, y, 6, 4));
    y += 4;
