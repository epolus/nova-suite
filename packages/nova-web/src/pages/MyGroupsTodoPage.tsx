...
const DEFAULT_COLS = ['number', 'title', 'priority', 'status', 'assigned_to_name', 'assignment_group_name', 'sla', 'updated_at'];
const PRESETS_KEY = 'nova_filter_presets_my_groups_incidents';
...
function buildColumns(listParams: Record<string, string>): DataColumnDef<Incident>[] {
  return [
    ...
    },
    {
      ...
      sortable: true,
      defaultVisible: true,
      render: (inc) => <span className="text-gray-500">{inc.assigned_to_name || 'Unassigned'}</span>,
    },
    {
...
function IncidentsTab() {
  ...
  useEffect(() => {
    setLoading(true);
    setSelectedIds([]);
    const apiParams: Record<string, string> = { my_groups: 'true' };
  ...
  const getListParams = useCallback((): Record<string, string> => {
    const lp: Record<string, string> = { my_groups: 'true' };
    ...
    <>
      ...
      {loading ? (
        ...
        <DataTable
          ...
          emptyMessage={params.search ? `No incidents matching "${params.search}"` : 'No open incidents in your groups.'}
...
export default function MyGroupsTodoPage() {
  ...
  useEffect(() => {
    requestsApi.taskQueue({ my_groups: 'true' }, 1, 1).then((res) =>
      setTaskCount(res.pagination.total),
    ).catch(() => {});
    incidentsApi.list({ my_groups: 'true' }, 1, 1).then((res) =>
  ...
  return (
    <>
      <PageHeader
        title="My Groups"
        description="Work items assigned to your assignment groups."
      />
      ...
      {activeTab === 'tasks' && <RequestTasksTab filterKey="my_groups" />}
    </>
  );
}
