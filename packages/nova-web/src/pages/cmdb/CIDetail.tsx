      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2 items-center">
            ...
            {canEdit && (
              <button
                onClick={() => guardNavigate(`/cmdb/${id}/edit`)}
                className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors"
              >
                {tActions('edit')}
              </button>
            )}
            <button onClick={() => guardNavigate('/cmdb')} className="px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">&larr; {tActions('back')}</button>
          </div>
        }
      />
