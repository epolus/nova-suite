...
export function useCIDetail() {
  ...
  const isRelFormDirty = showRelForm && (
    !!relForm.target || !!relForm.notes.trim() || relForm.type !== 'depends_on' || relForm.direction !== 'outgoing'
  );
  ...
  const {
    ...
    isDirty: isRelFormDirty,
    enabled: canEdit,
  });

  const navigateTo = useCallback((ciId: string) => {
    guardNavigate(`/cmdb/${ciId}`, { state: { listParams }, replace: true });
