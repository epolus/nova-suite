...
export default function RequestDetail() {
  ...
  const isNotesDirty = useMemo(
    () => notes.trim().length > 0 || Object.values(taskNotes).some((value) => value.trim().length > 0),
    [notes, taskNotes],
  );
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft' && prevId) {
        guardNavigate(`/requests/${prevId}`, { state: location.state });
      } else if (e.key === 'ArrowRight' && nextId) {
        guardNavigate(`/requests/${nextId}`, { state: location.state });
      }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [prevId, nextId, guardNavigate, location.state]);
        ...
        action={
          <div className="flex items-center gap-2">
            <button
              type="button"
              disabled={!prevId}
              onClick={() => prevId && guardNavigate(`/requests/${prevId}`, { state: location.state })}
            ...
            <button
              type="button"
              disabled={!nextId}
              onClick={() => nextId && guardNavigate(`/requests/${nextId}`, { state: location.state })}
              className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
