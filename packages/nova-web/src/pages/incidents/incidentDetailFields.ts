...
export function hasIncidentFormChanges(
  fields: IncidentFields,
  inc: Incident,
  linkedProblemIds: string[],
): boolean {
  if (Object.keys(buildIncidentUpdates(fields, inc)).length > 0) return true;
  const currentPrimaryProblemId = linkedProblemIds[0] || '';
  const nextPrimaryProblemId = fields.relatedProblemId || '';
  return currentPrimaryProblemId !== nextPrimaryProblemId;
}
...
export type NewIncidentFormState = {
  title: string;
  description: string;
  impact: string;
  urgency: string;
...
export function buildNewIncidentBaseline(defaultCallerId: string): NewIncidentFormState {
  return {
    title: '',
    description: '',
    impact: 'medium',
...
export function hasNewIncidentFormChanges(
  form: NewIncidentFormState,
  baseline: NewIncidentFormState,
): boolean {
