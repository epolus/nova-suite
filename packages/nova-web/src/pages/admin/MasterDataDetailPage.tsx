...
export default function MasterDataDetailPage<T extends { id: string; is_active: boolean }>({
  ...
  const isFormDirty = useMemo(
    ...
    [baselineActive, baselineForm, form, isActive],
  );
  ...
  const goToItem = useCallback((itemId: string) => {
    guardNavigate(`${basePath}/${itemId}`, { state: location.state });
  }, [basePath, guardNavigate, location.state]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft' && navInfo.prev) goToItem(navInfo.prev);
      if (e.key === 'ArrowRight' && navInfo.next) goToItem(navInfo.next);
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [isNew, navInfo.prev, navInfo.next, goToItem]);
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            {!isNew && (
              <>
                <button
                  ...
                  onClick={() => navInfo.prev && goToItem(navInfo.prev)}
                ...
                <button
                  ...
                  onClick={() => navInfo.next && goToItem(navInfo.next)}
                  className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
            ...
            <button
              type="button"
              onClick={() => guardNavigate(basePath)}
            ...
            <div key={field.key}>
              ...
              {field.type === 'textarea' ? (
                ...
                <select
                  ...
                  value={form[field.key] || ''}
                  onChange={(e) => setForm((prev) => ({ ...prev, [field.key]: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-hidden bg-white"
                >
