...
function formatDuration(start: string | null, end: string | null, emDash: string): string {
  if (!start) return emDash;
...
export default function WorkflowDetailPage() {
  const t = useTranslations('pages.admin.workflows.detail');
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : t('actionFailed'));
    } finally {
  ...
  if (!detail) return <p className="text-center text-gray-400 py-12">{t('notFound')}</p>;
  ...
  return (
    <>
      <PageHeader
        ...
        description={t('workflowType', { type: detail.type })}
        action={
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2">
          <Card>
            <h3 className="font-semibold text-gray-900 mb-4">{t('executionDetails')}</h3>
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <dt className="text-gray-500">{t('taskQueue')}</dt>
                <dd className="mt-0.5 font-mono text-gray-900">{detail.taskQueue || tTable('emDash')}</dd>
              </div>
              <div>
                <dt className="text-gray-500">{t('duration')}</dt>
                <dd className="mt-0.5 text-gray-900">{formatDuration(detail.startTime, detail.closeTime, tTable('emDash'))}</dd>
              </div>
              <div>
              ...
              <div>
                <dt className="text-gray-500">{t('closeTime')}</dt>
                <dd className="mt-0.5 text-gray-900">{formatDateTime(detail.closeTime)}</dd>
              </div>
              <div>
                <dt className="text-gray-500">{t('historyEvents')}</dt>
              ...
              {detail.parentExecution && (
                <div className="col-span-2">
                  <dt className="text-gray-500">{t('parentWorkflow')}</dt>
                  <dd className="mt-0.5">
                    <button
        ...
        <div>
          <Card>
            <h3 className="font-semibold text-gray-900 mb-4">{t('searchAttributes')}</h3>
            {Object.keys(detail.searchAttributes).length > 0 ? (
              ...
              <p className="text-sm text-gray-400">{t('noSearchAttributes')}</p>
            )}
            {detail.memo && Object.keys(detail.memo).length > 0 && (
              <>
                <h3 className="font-semibold text-gray-900 mt-4 mb-2">{t('memo')}</h3>
      ...
      <Card>
        <h3 className="font-semibold text-gray-900 mb-4">
          {t('history')} <span className="text-gray-400 font-normal text-sm">{t('eventsLoaded', { count: events.length })}</span>
        </h3>
        <div className="relative">
          ...
          {nextPageToken && (
            <div className="pl-10 pt-3">
              ...
              >
                {historyLoading ? tStates('loading') : t('loadMoreEvents')}
              </button>
            </div>
          )}
        </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
            <div className="px-6 py-5">
              <h2 className="text-lg font-bold text-gray-900 mb-2">
                {confirmAction === 'terminate' ? t('terminateTitle') : t('cancelTitle')}
              </h2>
              <p className="text-sm text-gray-600 mb-4">
                {confirmAction === 'terminate' ? t('terminateDescription') : t('cancelDescription')}
              </p>
              {confirmAction === 'terminate' && (
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-600 mb-1">{t('reasonOptional')}</label>
                  <input
                    type="text"
                    value={actionReason}
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                >
                  {actionLoading ? t('processing') : confirmAction === 'terminate' ? t('terminate') : t('cancelWorkflow')}
                </button>
              </div>
            </div>
          </div>
