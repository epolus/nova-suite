...
export default function UserDetailPage() {
  ...
  const [baseline, setBaseline] = useState(EMPTY_USER_FORM);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowLeft' && navInfo.prev) goToUser(navInfo.prev);
      if (e.key === 'ArrowRight' && navInfo.next) goToUser(navInfo.next);
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [isNew, navInfo.prev, navInfo.next, goToUser]);
            ...
            {!isNew && (
              <>
                <button
                  ...
                  onClick={() => navInfo.prev && goToUser(navInfo.prev)}
                ...
                <button
                  ...
                  onClick={() => navInfo.next && goToUser(navInfo.next)}
                  className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed"
                  title={t('nextUser')}
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
