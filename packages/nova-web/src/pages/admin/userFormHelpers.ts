...
function normalizeUserForm(form: UserFormState, isNew: boolean): UserFormState {
  return {
    ...form,
    password: isNew ? form.password : '',
...
export function hasUserFormChanges(
  form: UserFormState,
  baseline: UserFormState,
  isNew: boolean,
): boolean {
  return JSON.stringify(normalizeUserForm(form, isNew)) !== JSON.stringify(normalizeUserForm(baseline, isNew));
}
